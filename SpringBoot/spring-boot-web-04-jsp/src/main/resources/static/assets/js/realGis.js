...
function layerTypeEvent() {
    $('.layerTypeList .siteType').unbind('click').click(function (e) {
        ...
        if(layerType=='pollutantMap'){ //污染地图
            ...
            $('.regionArea').css('top','240px');
            $('.areaList ul').slideDown();
            $('.cityList ul').slideUp();
            $('.areaList ul li,.cityList ul li').removeClass('active');
            ...
            $('.mapSiteArea,.show-site-name,.air-factor').show();
            $('.dataTypeList li').removeClass('active');
            $('.realMonitorSite').show().siblings().hide();
            $('.areaList').hide();
            $('.regionArea').css('top','305px');
...
function factorClickEvent() {
    ...
    $('.air-factor ul li').click(function () {
        ...
        $('.factors-AQI img')[0].src="../../../assets/images/map/"+factorName+".png";
        if($('.pollutantMap').hasClass('active') && $('.dataTypeList li').hasClass('active')){
            ...
            $('.airSiteArea li').each(function () {
                deleteOverlays($(this).data('point'));
            });
            $.each(airSiteData,function (index,data) {
...
function addMapLabel(mapZoomLevel,layer){
    ...
    $.each(layer,function (index,item) {
        ...
        label.addEventListener('click',function () {
            var monitorTime = '';
            var monitorDate=$.format.date(item.MonitorTime,'yyyy-MM-dd HH:mm');
            var type = $('.dataTypeList li.active').data('type');
            if(type=='realData'){
                monitorTime=$.format.date(item.MonitorTime,'yyyy-MM-dd HH:mm');
            }else{
                var endTime = $.format.date(new Date(),'HH:00');
                monitorTime='00:00'+'~'+endTime;
                monitorDate=$.format.date(item.MonitorTime,'yyyy-MM-dd ')+$.format.date(new Date(),'HH:00');
            }
            var info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
                ...
                '<div class="chartTitle">AQI最近二十四小时变化趋势</div>'+
            ...
            infoBox.open(point);
            setTimeout(function () {
                drawAqiRealChart(item.OID,monitorDate);
            },100);
        });
        if(mapZoomLevel==1){
...
//获取点位趋势数据
function drawAqiRealChart(oid,monitorTime) {
    var zoomLevel=map.getZoom(),mapZoomLevel='';
    if($('.realMonitor').hasClass('active')){
        mapZoomLevel=3;
    }else{
        mapZoomLevel=zoomLevel<=8?1:zoomLevel>8 && zoomLevel<=10?2:3;
    }
    getDataByAjax('/gis/atmosphere/air/city/hour/24/trend',{RegionCode:oid,mapZoomLevel:mapZoomLevel,monitorTime:monitorTime},'GET',true,function (initData) {
...
function getEnpControlData() {
    ...
    var startTime =$.format.date( new Date(new Date(new Date().getTime() - 3600 * 1000 * 24)),'yyyy-MM-dd 00:00');
    var endTime =$.format.date( new Date(new Date(new Date().getTime() - 3600 * 1000 * 24)),'yyyy-MM-dd 23:59');
    getDataByAjax('/gis/atmosphere/enterprise/monitor',{statisticsType:type,startTime:startTime,endTime:endTime},'GET',true,function (initData) {
        if(initData.code==200 && initData.data && initData.data.length>0){
            $.each(initData.data,function (index,data) {
                mapEnpControlLabel(type,data);
            })
        }
    })
}
...
// 添加工业源类型点位
function mapEnpControlLabel(type,data) {
    var size=[38,38];
    ...
    window.curEntAndMatlsDic.put(data.psCode, {
        ...
        "click": function () {
            ...
            var infoBox = new BMapLib.InfoBox(window.map,getEnpControlInfo(type,data), {
                boxStyle: {
                    width: type==1?"480px":"400px",
                    Height: "340px",
                    backgroundColor: "white"
                },
...
function getEnpControlInfo(type,data) {
    for(var key in data){
    ...
    var info = '';
    if(type==1){ //超标
        //var endTime = $.format.date(new Date(),'yyyy-MM-dd HH:mm:ss');
        //var startTime = $.format.date(new Date(new Date(new Date().getTime() - 1000*60*60)),'yyyy-MM-dd HH:mm:ss');
        var startTime =$.format.date( new Date(new Date(new Date().getTime() - 3600 * 1000 * 24)),'yyyy-MM-dd 00:00');
        var endTime =$.format.date( new Date(new Date(new Date().getTime() - 3600 * 1000 * 24)),'yyyy-MM-dd 23:59');
        var tbody ='';
        getDataByAjax('/gis/atmosphere/enterprise/cs/desc',{pscode:data.psCode,startTime:startTime,endTime:endTime},'GET',false,function (initData) {
            if(initData.code==200 && initData.data && initData.data.length>0){
                $.each(initData.data, function (i, itemData) {
                    ...
                    tbody += '<tr>' +
                        ...
                        '<td>' + itemData.standardvalue + '</td>' +
                        '<td>' + itemData.avgstrength + '</td>' +
                        '<td>' + itemData.overtimes + '</td>' +
                        '</tr>';
                });
            }else{
                tbody += "<tr><td colspan='6' style='text-align: center'>暂无数据</td></tr>";
            }
        });
        info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
            '<div class="map-air-quality-popup map-popup-model">' +
            '<div class="header modelTitle"><h4 class="text-center">点位详情</h></div>' +
            '<div class="popup-info"><div class="siteDataArea"><div class="chartArea"><table><thead>' +
            '<tr><th style="width: 16%;">企业</th><th style="width: 16%;">排放口名称</th>' +
            '<th style="width: 14%;">污染物</th><th style="width: 18%;">排放标准(mg/m³)</th>' +
            '<th style="width: 16%;">累计超标日均值(mg/m³)</th><th style="width: 16%;">超标倍数</th>' +
            '</tr></thead><tbody id="exceedDataTable">'+tbody+'</tbody>'+
            '</table></div></div></div>' +
            '</div></div></div>' +
            '<div class="popup-tip-container"><div class="popup-tip"></div></div>' +
            '</div>';
        ...
        info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
            ...
            '<li><span class="li-style">企业名称：</span><span class="li-text">'+data.psName+'</span></li>' +
            '<li><span class="li-style">预警前排放量(kg)：</span><span class="li-text">'+data.pflF+'</span></li>' +
            '<li><span class="li-style">排放量(kg)：</span><span class="li-text">'+data.pfl+'</span></li>' +
            '<li><span class="li-style">增加量(kg)：</span><span class="li-text">'+data.bhl+'</span></li>' +
            '<li><span class="li-style">与预警前变化(%)：</span><span class="li-text">'+data.bh+'</span></li>' +
            '<li><span class="li-style">累计天数：</span><span class="li-text">'+data.pscnt+'</span></li>' +
            '<li><span class="li-style">企业管控类型：</span><span class="li-text">'+data.psType+'</span></li>' +
            '<li><span class="li-style">管控类型：</span><span class="li-text">'+data.gklx+'</span></li>' +
            '<li><span class="li-style">具体措施：</span><span class="li-text">'+data.jtcs+'</span></li>' +
            '</ul></div></div>' +
            '</div></div></div>' +
            '<div class="popup-tip-container"><div class="popup-tip"></div></div>' +
            '</div>';
        ...
        info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
            ...
            '<li><span class="li-style">管控类型：</span><span class="li-text">'+data.gklx+'</span></li>' +
            '<li><span class="li-style">累计天数：</span><span class="li-text">'+data.pscnt+'</span></li>' +
            '<li><span class="li-style">管控措施：</span><span class="li-text">'+data.gkcs+'</span></li>' +
            '<li><span class="li-style">排放量(kg)：</span><span class="li-text">'+data.pfl+'</span></li>' +
            '<li><span class="li-style">具体管控措施：</span><span class="li-text">'+data.jtcs+'</span></li>' +
            '</ul></div></div>' +
            '</div></div></div>' +
            '<div class="popup-tip-container"><div class="popup-tip"></div></div>' +
            '</div>';
    }else if(type==4){  //限产不到位
        info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
            '<div class="map-air-quality-popup map-popup-model">' +
            '<div class="header modelTitle"><h4 class="text-center">点位详情</h></div>' +
            '<div class="popup-info"><div class="siteDataArea"><ul>' +
            ...
            '<li><span class="li-style">企业管控类型：</span><span class="li-text">'+data.psType+'</span></li>' +
            '<li><span class="li-style">管控类型：</span><span class="li-text">'+data.gklx+'</span></li>' +
            '<li><span class="li-style">管控减排比例(%)：</span><span class="li-text">'+data.bh+'</span></li>' +
            '<li><span class="li-style">具体措施：</span><span class="li-text">'+data.jtcs+'</span></li>' +
            '</ul></div></div>' +
            '</div></div></div>' +
            '<div class="popup-tip-container"><div class="popup-tip"></div></div>' +
...
function getAirMapData(siteCode,siteType) {
    ...
    getDataByAjax('/gis/atmosphere/air/site/type/record',{siteType:''},'GET',true,function (initData) {
        if(initData.code==200 && initData.data && initData.data.length>0){
            ...
            window.airSiteData = initData.data;
        }
    })
}
...
function mapAirLabel(data) {
    ...
    window.curEntAndMatlsDic.put(data.OID, {
        ...
        "click": function () {
            ...
            infoBox.open(tmpMarker);   //图片加载完毕重绘infowindow
            setTimeout(function () {
                drawAqiRealChart(data.OID,data.MonitorTime);
            },100);
        }
    });
    //定义点位点击触发事件
...
function initPollutantMap(){
    // 初始化污染源地图点位数据
    var typeSelected = $(".pollutantSiteArea li.active").length;
    ...
    //污染源点击事件
    $(".pollutantType .item-pollutant").click(function (e) {
        e.stopPropagation();
        $(this).toggleClass('active');
        if(!$(this).hasClass('active')){
    ...
    // 污染源数据类型点击事件
    $(".pollutantSiteArea li").click(function (e) {
        e.stopPropagation();
        infoBoxList.forEach(function (infobox) {  infobox.close(); });
        $(this).toggleClass('active');
...
function getPollutantMapData() {
    ...
    getDataByAjax('/gis/atmosphere/waste/monitor/enterprise',{RegionCode:'',psname:'',startTime:startTime,endTime:endTime},'GET',true,function (initData) {
        if(initData.code==200 && initData.data && initData.data.length>0){
           ...
           var points = [],exceedPoints = [];
           var options = {
                size: BMAP_POINT_SIZE_BIG,
                shape: BMAP_POINT_SHAPE_STAR, //BMAP_POINT_SHAPE_STAR、BMAP_POINT_SHAPE_SQUARE 、BMAP_POINT_SHAPE_CIRCLE、BMAP_POINT_SHAPE_RHOMBUS
                color:'#29da4e'
            };
            var options2 = {
                size: BMAP_POINT_SIZE_BIG,
            ...
            $.each(initData.data,function (index,data) {
                if(data.IsOverProof=='0'){
                    points.push(new BMap.Point(data.Longitude, data.Latitude));
                }else if(data.IsOverProof == '1'){
                    exceedPoints.push(new BMap.Point(data.Longitude, data.Latitude));
                }
            });
            mapPollutionLabel(type,siteData,points,options);//添加正常废气企业海量点
            mapPollutionLabel(type,siteData,exceedPoints,options2); //添加超标海量点
        }
    })
}

function mapPollutionLabel(type,siteData,points,options) {
    var pointCollection = new BMap.PointCollection(points, options);
    pointCollection.addEventListener('click', function (e) {
        if($('[data-point='+type+']').hasClass('active')){
            var data = '';//弹窗信息
            //循环查出值
            for (var i = 0; i < siteData.length; i++) {
                if(siteData[i].Longitude==e.point.lng&&siteData[i].Latitude==e.point.lat && e.currentTarget.siteType==type){//经度==点击的,维度
                    data = siteData[i];
                    break;
                }
            }
            ...
            var info = '<div class="popup"><div class="popup-content-wrapper"><div class="popup-content">' +
                ...
                '<li class="site-trend active icon-blue"  data-code='+data.PSCode+' data-type="site-chart" data-chart="chartData">排放量趋势</li>' +
            ...
            lastInfoBox = infoBox;
            infoBox.open(tmpPt);
            getPollutantChartData('exceedData',$('.site-data-detailbtn [data-chart="exceedData"]').data('code'));//获取废气超标数据
            getPollutantChartData('realData',$('.site-data-detailbtn [data-chart="realData"]').data('code'));//获取废气最新数据
...
function getPollutantChartData(type,pscode) {
    ...
    if(type=='realData'){
        ...
        startTime = $.format.date(new Date(new Date(new Date().getTime() - 1000*60*60*24*6)),'yyyy-MM-dd');
        endTime = $.format.date(new Date(),'yyyy-MM-dd');
        //410800000366
        $.get('/gis/atmosphere/ps/day/list?startTime='+startTime+'&endTime='+endTime+'&psCode='+pscode,function (initData) {
            if(initData.code == 200 && initData.data && initData.data.length>0){
...
//   添加指定的Overlays
function addOverlays(content) {
    var allOverlay  = map.getOverlays();
    var labelContent="";
    if (allOverlay.length) {
        for (var i=0; i < allOverlay.length; i++) {
            if(allOverlay[i].toString() == "[object Marker]" || allOverlay[i].toString() == "[object PointCollection]"){
